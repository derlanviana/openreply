  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted mt-1">
            {data.provider !== "ZERNIO" && data.requestedCount === "all" ? "Todos os posts" : "Recentes"}:{" "}
            {totals.posts} {totals.posts === 1 ? "post" : "posts"} de @
      ...
      {!insightsAvailable && (
        <div className="panel rounded p-4 border border-border">
          <p className="text-sm text-foreground">
            Visualizações, alcance, salvamentos e compartilhamentos precisam da permissão de insights.
          </p>
          <p className="text-sm text-muted mt-1">
            Reconecte sua conta para liberar. Enquanto isso, mostramos curtidas e
            comentários.
          </p>
          <a
            href="/api/instagram/connect"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        ...
        <StatCard label="Salvamentos" value={formatNumber(totals.saved)} />
      ...
      <div className="panel rounded p-4 sm:p-6">
        ...
        {posts.length === 0 ? (
          <p className="text-sm text-muted py-8 text-center">Nenhum post encontrado</p>
        ) : (
            ...
            <table className="w-full min-w-[720px] text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wide text-zinc-500 border-b border-border">
                  ...
                  <th className="py-2 px-3 font-medium text-right">Compartilh.</th>
