...
export default function CampaignDetailPage() {
  ...
  const metrics = [
    ...
    { label: "Taxa de cliques", value: `${campaign.analytics.ctr}%` },
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,340px)_1fr]">
      ...
      <div className="space-y-6">
        ...
        <Summary title="E o comentário tiver">
          ...
          {campaign.dmTriggerEnabled && (
            <p className="text-xs text-muted">
              Também responde quando alguém mandar DM com{" "}
              {campaign.matchAnyWord ? "qualquer texto" : "estas palavras"}.
            </p>
          )}
        ...
        {campaign.requireFollow && (
          <Summary title="A pessoa precisa seguir antes">
            <FieldBox>
              {campaign.followPromptMessage ||
                DEFAULT_FOLLOW_PROMPT_MESSAGE}
            </FieldBox>
            <FieldBox>
              {campaign.followPromptButtonLabel || DEFAULT_FOLLOW_BUTTON_LABEL}
            </FieldBox>
          </Summary>
        )}

        <Summary title="Depois, a pessoa recebe esta DM">
          <FieldBox>{campaign.dmMessage}</FieldBox>
          {hasLink && (
            <FieldBox>{campaign.linkButtonLabel || DEFAULT_LINK_BUTTON_LABEL}</FieldBox>
          )}
          {hasSecondLink && (
            <FieldBox>
              {campaign.trackedLinks?.[1]?.label || DEFAULT_LINK_BUTTON_LABEL}
            </FieldBox>
          )}
        </Summary>

        {hasLink && (
          <Summary title="Link enviado">
        ...
        {campaign.followUpEnabled && campaign.followUpMessage && (
          <Summary title="Em seguida, uma mensagem de acompanhamento">
      ...
      <div className="space-y-4">
        ...
        {tab === "preview" && (
          ...
          <CampaignPreview
            ...
            postThumb={postThumb}
            caption=""
            sampleComment={campaign.matchAnyWord ? "adorei!" : campaign.keywords[0] ?? "LINK"}
            ...
            hasLink={hasLink}
            linkButtonLabel={campaign.linkButtonLabel ?? DEFAULT_LINK_BUTTON_LABEL}
            linkUrl={
