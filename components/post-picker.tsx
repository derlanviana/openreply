    ...
    fetch(`/api/instagram/posts${params.size ? `?${params}` : ""}`)
      ...
      .then((data) => {
        ...
        if (data.success) {
          ...
          setError(data.error ?? "Não foi possível carregar os posts");
        }
      })
      .catch(() => {
        if (!cancelled && !cached.data) setError("Não foi possível carregar os posts");
      })
      .finally(() => {
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between gap-2">
        <input
          ...
          placeholder="Buscar posts pela legenda…"
      ...
      {visible.length === 0 ? (
        ...
        <>
          ...
          <button
            ...
            title={isUsed ? `Já usado na campanha "${usedByName}"` : undefined}
            className={`
