...
export default function CampaignBuilder({ mode, campaignId }: CampaignBuilderProps) {
  ...
  const [linkButtonLabel, setLinkButtonLabel] = useState(DEFAULT_LINK_BUTTON_LABEL);
  const [secondLinkOpen, setSecondLinkOpen] = useState(false);
  const [secondaryDestinationUrl, setSecondaryDestinationUrl] = useState("");
  const [secondaryButtonLabel, setSecondaryButtonLabel] = useState(DEFAULT_LINK_BUTTON_LABEL);
  const [requireFollow, setRequireFollow] = useState(false);
  const [followPromptMessage, setFollowPromptMessage] = useState("");
  const [followPromptButtonLabel, setFollowPromptButtonLabel] =
    useState(DEFAULT_FOLLOW_BUTTON_LABEL);
  ...
  useEffect(() => {
    ...
    fetch("/api/automations", { cache: "no-store" })
      ...
      .then((payload) => {
        ...
        setSecondaryButtonLabel(secondLink?.label ?? DEFAULT_LINK_BUTTON_LABEL);
  ...
  async function handleSubmit(activeValue: boolean) {
    ...
    if (triggerScope === "specific" && !postId)
      return setError("Escolha um post ou reel para disparar a campanha.");
    if (matchMode === "specific" && keywords.length === 0)
      return setError("Adicione pelo menos uma palavra-chave ou escolha qualquer palavra.");
    if (!dmMessage.trim()) return setError("Escreva a DM com o link.");
    if (openingDmEnabled && (!openingDmMessage.trim() || !openingDmButtonLabel.trim()))
      return setError("A DM de abertura precisa de uma mensagem e do texto do botão.");
    ...
    const payload = {
      ...
      secondaryButtonLabel: secondaryButtonLabel.trim() || DEFAULT_LINK_BUTTON_LABEL,
      requireFollow,
      ...
      {importQueue && (
        <div className="rounded border border-accent/30 bg-accent/5 px-4 py-3 text-sm">
          ...
          <span className="text-muted">
            Os campos vieram preenchidos do seu CSV. Escolha o reel, ajuste o que quiser e
            salve para carregar a próxima. Se não quiser esta, clique em Pular.
          </span>
        </div>
      )}
      ...
      <div className="space-y-8 min-w-0">
        ...
        <div className="space-y-3">
          ...
          <input
            ...
            placeholder="ex: Lançamento Vila Mariana"
        ...
        <Section title="E o comentário tiver">
          ...
          {matchMode === "specific" && (
            <div className="space-y-1">
              <input
                ...
                className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
              />
              <p className="text-xs text-muted">Separe as palavras com vírgula</p>
            </div>
          )}
          <Radio
          ...
          <div className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2.5">
            <span className="text-sm text-foreground">
              ...
              {matchMode === "any" ? "qualquer texto" : "estas palavras"}
            </span>
            <Toggle
              on={dmTriggerEnabled}
          ...
          {dmTriggerEnabled && (
            <p className="text-xs text-muted">
              {matchMode === "any"
                ? "Toda DM recebida por esta conta vai receber a resposta abaixo. Use com cuidado."
                : "Uma DM com qualquer uma dessas palavras recebe a mesma resposta, sem precisar comentar."}
            </p>
          )}
          ...
          {publicReplyEnabled && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted">
                Uma delas é sorteada a cada vez, para as respostas não ficarem
                todas iguais.
              </p>
            </div>
          )}
            ...
            {openingDmEnabled && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Oi! Que bom que você chegou até aqui."
                  rows={3}
            ...
            {requireFollow && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder={DEFAULT_FOLLOW_PROMPT_MESSAGE}
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
                ...
                />
                <p className="text-xs text-muted">
                  O link só é enviado depois que a pessoa toca no botão e o Instagram
                  confirma que ela segue a conta. Se não der para confirmar, o link
                  é enviado mesmo assim.
                </p>
              </div>
            )}
          </div>
        ...
        <Section title="Depois, a pessoa recebe">
          <div className="rounded-lg border border-border p-3 space-y-2">
            <span className="text-sm text-foreground">uma DM com link</span>
            <textarea
              value={dmMessage}
            ...
            {linkOpen ? (
              <div className="space-y-2">
                <input
                  ...
                  placeholder="https://seulink.com.br/oferta"
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
                />
                <input
                ...
                {secondLinkOpen ? (
                  <div className="space-y-2 border-t border-border pt-2">
                    <input
                      ...
                      placeholder="https://seulink.com.br/segundo"
                      className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
                    />
                    <input
            ...
            <p className="text-xs text-muted">
              {"{link}"} insere o link rastreado e {"{username}"} coloca o nome da pessoa.
            </p>
          </div>
          <div className="mt-3 rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
            ...
            {followUpEnabled && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  placeholder="Passando para agradecer por me seguir. Obrigado pelo apoio!"
                  rows={3}
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                  maxLength={1000}
                />
                ...
                <p className="text-xs text-muted">
                  {followUpDelayMinutes > 0
                    ? `Enviada ${followUpDelayMinutes} min depois que a pessoa abrir o link.`
                    : "Enviada logo depois que a pessoa abrir o link."}
                  {" {username}"} coloca o nome da pessoa. Máximo de 24 horas, para ficar
      ...
      <div>
        ...
        <div className="flex min-w-0 justify-center lg:sticky lg:top-6 lg:block">
          <CampaignPreview
            ...
            linkButtonLabel={linkButtonLabel || DEFAULT_LINK_BUTTON_LABEL}
            linkUrl={trackedDestinationUrl.trim() || undefined}
            hasSecondLink={
              secondLinkOpen && Boolean(secondaryDestinationUrl.trim())
            }
            secondLinkButtonLabel={secondaryButtonLabel || DEFAULT_LINK_BUTTON_LABEL}
            requireFollow={requireFollow}
            followPromptMessage={followPromptMessage}
            followPromptButtonLabel={followPromptButtonLabel || DEFAULT_FOLLOW_BUTTON_LABEL}
